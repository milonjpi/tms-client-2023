...
import { useAddPaperWorkMutation } from 'store/api/paperWork/paperWorkApi';
...
const AddPaperWork = ({ open, handleClose, paperType }) => {
  ...
  const [addPaperWork] = useAddPaperWorkMutation();
  ...
  const onSubmit = async (data) => {
    ...
    try {
      setLoading(true);
      const res = await addPaperWork({ ...newData }).unwrap();
      if (res.success) {
        handleClose();
        setLoading(false);
        reset();
