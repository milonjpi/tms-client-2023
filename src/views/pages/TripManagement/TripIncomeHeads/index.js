...
import AddTripIncomeHead from './AddTripIncomeHead';
import TripIncomeHeadRow from './TripIncomeHeadRow';

const TripIncomeHeads = () => {
  ...
  const { data, isLoading } = useGetIncomeHeadsQuery(
    { ...query },
    { refetchOnMountOrArgChange: true }
  );

  const allTripIncomeHeads = data?.incomeHeads || [];
    ...
    >
      ...
      <Box sx={{ overflow: 'auto' }}>
        <Table sx={{ minWidth: 400 }}>
          ...
          <TableBody>
            {allTripIncomeHeads?.length ? (
              allTripIncomeHeads.map((item) => (
                <TripIncomeHeadRow key={item.id} sn={sn++} data={item} />
              ))
            ) : (
              <StyledTableRow>
...
export default TripIncomeHeads;
