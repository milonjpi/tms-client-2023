...
import { useGetTripExpensesQuery } from 'store/api/tripExpense/tripExpenseApi';
...
const TripExpense = () => {
  ...
  const { data, isLoading } = useGetTripExpensesQuery(
    { ...query },
    { refetchOnMountOrArgChange: true }
  );

  const allTripExpenses = data?.tripExpenses || [];
  ...
  return (
    ...
    >
      ...
      <AddTripExpense open={open} handleClose={() => setOpen(false)} />
      {/* end popup items */}
      <Box sx={{ overflow: 'auto' }}>
        <Table sx={{ minWidth: 600 }}>
          ...
          <TableBody>
            {allTripExpenses?.length ? (
              allTripExpenses.map((item) => (
                <TripExpenseRow key={item.id} sn={sn++} data={item} />
              ))
            ) : (
              <StyledTableRow>
                <StyledTableCell colSpan={15} sx={{ border: 0 }} align="center">
                  {isLoading ? (
